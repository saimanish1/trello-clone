...
import {addColumn, getAllCards, reorderCard, reorderList} from "../../store/actions/listActions";
        ...
        if (this.state.isNewColumnTitleInEdit) {
            return (
                <div className={classes.add_list__wrapper}>
                    <input type={"text"} value={this.state.newColumnTitle} placeholder={"Enter list title..."}
                           className={classes.list_title__inedit} onChange={this.onColumnTitleChangeHandler}
                           onKeyDown={this.onKeyDownColumnTitleHandler}/>
                    <div className={classes.add_list__controls}>
                        <button className={classes.add_list__controls__button}
                                onClick={this.onKeyDownColumnTitleHandler} name="addcard">Add a list
                        </button>
            ...
            return (
                <button className={classes.add_list} onClick={this.onAddColumnHandler}>
                    ...
                    <span
                        className={classes.add_list__text}>
                        {this.props.list.columnOrder.length > 0 ? 'Add another list' : 'add a list'}
                        </span>
                </button>
            )
        }
    ...
    onDragEnd = (result) => {
        ...
        if (type === 'column') {
            if (source.index !== destination.index) {
                this.props.reorderList({...payload, columnOrder: this.props.list.columnOrder}, this.props.boardId);
                return;
            }
            ...
            if (this.props.list.columnOrder.length === 0) {
                listData = (
                    <React.Fragment>
                        ...
                        <div style={{width: '30rem'}}><Backdrop
                            clickedOutside={this.onColumnTitleOutsideClicked}>{this.addColumnWrapper()}</Backdrop></div>
                    </React.Fragment>)
            }
            else {
                listData = (
                    ...
                    >
                        ...
                        >
                            {(provided) => (
                                <div className={classes.listcontainer} {...provided.droppableProps}
                                     ref={provided.innerRef}>
                                    {this.props.list.columnOrder.map((columnId, index) => {

                                        const column = this.props.list.columns[columnId];
                                        const cards = column.cardIds.map(cardId => this.props.cards.cards[cardId])
                                    ...
                                    <Backdrop
                                        clickedOutside={this.onColumnTitleOutsideClicked}>{this.addColumnWrapper()}</Backdrop>
...
export default connect(mapStateToProps, {reorderCard, reorderList, getAllCards, addColumn})(List);